...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case mine:
        return mineCase(choice1, choice2, state, handPos, currentPlayer);
    ...
    case baron:
        return baronCase(choice1, state, handPos, currentPlayer);
    ...
    case minion:
        return minionCase(choice1, choice2, state, handPos, currentPlayer);
    ...
    case ambassador:
        return ambassadorCase(choice1, choice2, state, handPos, currentPlayer);
...
// End of initial base code; Begin card specific refactorization

// NOTE: FUNCTION PROTOTYPES PLACED IN dominion.h
    ...
    if (choice1 > 0){                                                           //Boolean true or going to discard an estate
        ...
        for(p = 0; p <= state->hand[currentPlayer][p]; p++){
            if (state->hand[currentPlayer][p] == estate){                       //Found an estate card!
                ...
                estate_in_hand = 1;                                             //Estate card found.
                break;
            }
        }
    }

    else if(choice1 == 0 || estate_in_hand == 0){
...
int ambassadorCase(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer){
    ...
    for (i = 0; i < state->numPlayers; i++){
        if (i != currentPlayer){
            gainCard(state->hand[currentPlayer][choice1], state, rand() % 3, i);
        }
    }
...
int tributeCase(struct gameState *state, int currentPlayer, int nextPlayer){
    ...
    while(validTributeReveals < cardsToReveal){
        ...
        if (state->deckCount[nextPlayer] == 0){
            ...
            //Transfer discard pile to deck pile
            ...
            //Zero out discardCount and shuffle deck
            state->discardCount[nextPlayer] = 0;
            shuffle(nextPlayer,state);
        }
        
        //Add deck card to tribute array, add revealed card to discard pile, increment discard count,
        //set end deck position to -1, decrement number or cards in deck
        tributeRevealedCards[validTributeReveals] = state->deck[nextPlayer][state->deckCount[nextPlayer] -1];
        ...
        //Increment number of valid reveals
        validTributeReveals++;
    }
...
int mineCase(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer){
    ...
    for (i = 0; i < state->handCount[currentPlayer]; i++){
        ...
        if (state->hand[currentPlayer][i] == removeCard){
            discardCard(i, currentPlayer, state, 1);
            break;
        }
    }

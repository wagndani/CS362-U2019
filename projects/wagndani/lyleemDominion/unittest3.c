...
int main(){
    ...
    baseGame.hand[checkPlayer][baseGame.handCount[checkPlayer] - 1] = duchy;
    baseGame.hand[checkPlayer][baseGame.handCount[checkPlayer] - 2] = duchy;
    ...
    choice2 = -1;
    nb_assert(executeAmbassadorCard(choice1, choice2, &testGame, handPos, checkPlayer), -1);
    ...
    choice2 = 1;
    choice1 = baseGame.handCount[checkPlayer] -1;
    int checkcard = baseGame.hand[checkPlayer][choice1];
    memcpy(&testGame, &baseGame, sizeof(struct gameState));
    nb_assert(executeAmbassadorCard(choice1, choice2, &testGame, handPos, checkPlayer), 0);
    
    printf("\nCheck requested number of cards were sent back to supply by checking supply count of choice1 card\n");
    nb_assert(supplyCount(checkcard, &testGame), supplyCount(checkcard, &baseGame) + choice2 - baseGame.numPlayers + 1);
    ...
    for(i = 0; i < baseGame.deckCount[checkPlayer + 1]; i++){
        if(baseGame.deck[checkPlayer + 1][i] == checkcard){
            nextPlayerChoice1Count_Base++;
        }
    }
    
    for(i = 0; i < baseGame.discardCount[checkPlayer + 1]; i++){
        if(baseGame.discard[checkPlayer + 1][i] == checkcard){
            nextPlayerChoice1Count_Base++;
        }
    }
    
    for(i = 0; i < baseGame.handCount[checkPlayer + 1]; i++){
        if(baseGame.hand[checkPlayer + 1][i] == checkcard){
            nextPlayerChoice1Count_Base++;
        }
    }
    ...
    for(i = 0; i < testGame.deckCount[checkPlayer + 1]; i++){
        if(testGame.deck[checkPlayer + 1][i] == checkcard){
            nextPlayerChoice1Count_Test++;
        }
    }
    
    for(i = 0; i < testGame.discardCount[checkPlayer + 1]; i++){
        if(testGame.discard[checkPlayer + 1][i] == checkcard){
            nextPlayerChoice1Count_Test++;
        }
    }
    
    for(i = 0; i < testGame.handCount[checkPlayer + 1]; i++){
        if(testGame.hand[checkPlayer + 1][i] == checkcard){
            nextPlayerChoice1Count_Test++;
        }
    }
